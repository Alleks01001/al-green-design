...
type RenderPreset = 'editor' | 'presentation' | 'photoreal';
type EnvironmentPreset = 'clear' | 'golden' | 'overcast' | 'night';
type ToneMappingMode = 'aces' | 'reinhard' | 'cineon' | 'linear';
type CameraMode = 'orbit' | 'walk' | 'top' | 'architecture' | 'garden';
...
export default function LandscapePlatform() {
  ...
  const [status, setStatus] = useState('Bereit: V0.40 PHOTOREAL RENDER CORE – plastische Werkzeuge, dreidimensionales Markenlogo und Premium-Studio.');
  ...
  const [renderPreset, setRenderPreset] = useState<RenderPreset>('presentation');
  const [environmentPreset, setEnvironmentPreset] = useState<EnvironmentPreset>('clear');
  const [toneMappingMode, setToneMappingMode] = useState<ToneMappingMode>('aces');
  const [renderExposure, setRenderExposure] = useState(1.08);
  const [environmentStrength, setEnvironmentStrength] = useState(0.9);
  const [cameraFov, setCameraFov] = useState(50);
  const [contactShadows, setContactShadows] = useState(true);
  const [reflectiveGround, setReflectiveGround] = useState(true);
  const [renderHaze, setRenderHaze] = useState(0.45);
...
Annahmen: ${result.assumptions.join(' · ')}`
  ...
  function buildPrintableReportHtml() {
    ...
    return `<!doctype html><html lang="de"><head><meta charset="utf-8"><title>${projectInfo.name} – AL Green Design Bericht</title><style>
      ...
      <header><div class="brand"><div class="logo">AL</div><div><h1>AL Green Design</h1><p>Landscape Architecture Studio · Projektbericht V0.40</p></div></div></header>
  ...
  return (
    <section className={`platform ${presentationMode?'presentationMode':''}`}>
      <aside className="panel">
            ...
            <div className="form">
              ...
              <label>Kamera
                <select value={cameraMode} onChange={e=>setCameraMode(e.target.value as any)}>
                  ...
                  <option value="architecture">Architektur-Perspektive</option>
                  <option value="garden">Gartenweg-Kamera</option>
        ...
        {tab === 'scene' && (
          <>
            <h2>Photoreal Render Core</h2>
            <div className="renderPresetGrid">
              {([
                ['editor','Editor','Schnell und klar'],
                ['presentation','Präsentation','Weiche Schatten und PBR'],
                ['photoreal','Fotoreal','Maximale Licht- und Materialqualität']
              ] as const).map(([value,label,description])=>(
                <button key={value} className={`renderPresetCard ${renderPreset===value?'active':''}`} onClick={()=>{
                  setRenderPreset(value);
                  if(value==='editor'){setPerformanceMode('fast');setContactShadows(false);setReflectiveGround(false);setRenderExposure(1.0);}
                  if(value==='presentation'){setPerformanceMode('quality');setContactShadows(true);setReflectiveGround(true);setRenderExposure(1.08);}
                  if(value==='photoreal'){setPerformanceMode('quality');setContactShadows(true);setReflectiveGround(true);setRenderExposure(1.16);}
                }}>
            ...
            <div className="renderControlPanel">
              <label>Umgebung
                <select value={environmentPreset} onChange={e=>{const value=e.target.value as EnvironmentPreset;setEnvironmentPreset(value);setNightMode(value==='night');}}>
                  <option value="clear">Klarer Tag</option>
                  <option value="golden">Goldene Stunde</option>
                  <option value="overcast">Bewölkt / weich</option>
                  <option value="night">Nachtbeleuchtung</option>
                </select>
              </label>
              <label>Tone Mapping
                <select value={toneMappingMode} onChange={e=>setToneMappingMode(e.target.value as ToneMappingMode)}>
              ...
              <label>Belichtung <span>{renderExposure.toFixed(2)}</span>
                <input type="range" min="0.55" max="1.8" step="0.01" value={renderExposure} onChange={e=>setRenderExposure(Number(e.target.value))}/>
              </label>
              <label>Umgebungsreflexion <span>{environmentStrength.toFixed(2)}</span>
                <input type="range" min="0" max="2" step="0.05" value={environmentStrength} onChange={e=>setEnvironmentStrength(Number(e.target.value))}/>
              </label>
              <label>Kamera-Brennweite <span>{cameraFov}°</span>
                <input type="range" min="28" max="75" step="1" value={cameraFov} onChange={e=>setCameraFov(Number(e.target.value))}/>
              </label>
              <label>Atmosphärische Tiefe <span>{renderHaze.toFixed(2)}</span>
                <input type="range" min="0" max="1" step="0.05" value={renderHaze} onChange={e=>setRenderHaze(Number(e.target.value))}/>
              </label>
            </div>

            <div className="cameraPresetRow">
              <button className={`btn ${cameraMode==='architecture'?'active':''}`} onClick={()=>setCameraMode('architecture')}>Architektur</button>
              <button className={`btn ${cameraMode==='garden'?'active':''}`} onClick={()=>setCameraMode('garden')}>Gartenweg</button>
              <button className={`btn ${cameraMode==='orbit'?'active':''}`} onClick={()=>setCameraMode('orbit')}>Gesamtansicht</button>
              <button className={`btn ${cameraMode==='top'?'active':''}`} onClick={()=>setCameraMode('top')}>Vogelperspektive</button>
            </div>

            <div className="renderToggleGrid">
              <label><input type="checkbox" checked={enable3DShadows} onChange={e=>setEnable3DShadows(e.target.checked)}/> Hochwertige Schatten</label>
              <label><input type="checkbox" checked={contactShadows} onChange={e=>setContactShadows(e.target.checked)}/> Kontakt-Schatten</label>
              <label><input type="checkbox" checked={reflectiveGround} onChange={e=>setReflectiveGround(e.target.checked)}/> Bodenreflexionen</label>
              <label><input type="checkbox" checked={showGrid3D} onChange={e=>setShowGrid3D(e.target.checked)}/> 3D-Raster</label>
            </div>

            <button className="btn primary renderCaptureButton" onClick={()=>window.dispatchEvent(new CustomEvent('algreen-capture-render'))}>Aktuelle 3D-Ansicht als PNG speichern</button>

            <h2 style={{marginTop:16}}>Importierte 3D-Modelle</h2>
            <div className="list">
      ...
      <div className="workspace">
        <div className="topbar">
          <div className="topbarBrand3D"><BrandLogo3D compact={true} animated={false} /><span><strong>V0.40</strong><small>PHOTOREAL RENDER CORE</small></span></div>
...
function toneMappingForMode(mode: ToneMappingMode) {
  if (mode === 'reinhard') return THREE.ReinhardToneMapping;
  if (mode === 'cineon') return THREE.CineonToneMapping;
  if (mode === 'linear') return THREE.LinearToneMapping;
  return THREE.ACESFilmicToneMapping;
}

function createProceduralEnvironment(renderer: THREE.WebGLRenderer, preset: EnvironmentPreset, nightMode: boolean): ProceduralEnvironment {
  const canvas = document.createElement('canvas');
  canvas.width = 1024;
  canvas.height = 512;
  const ctx = canvas.getContext('2d');
  if (!ctx) throw new Error('Umgebungstextur konnte nicht erzeugt werden.');

  const palettes: Record<EnvironmentPreset,{top:string;middle:string;bottom:string;sun:string;fog:string;sunX:number;sunY:number}> = {
    clear:{top:'#6fa8dc',middle:'#d9efff',bottom:'#f5ead8',sun:'#fff5cf',fog:'#dceaf1',sunX:720,sunY:145},
    golden:{top:'#5d678e',middle:'#e8a36f',bottom:'#f7d8ae',sun:'#fff0b3',fog:'#d5b29f',sunX:760,sunY:250},
    overcast:{top:'#82909d',middle:'#c3ccd1',bottom:'#e7e3dc',sun:'#ffffff',fog:'#c7d0d4',sunX:680,sunY:180},
    night:{top:'#050916',middle:'#172340',bottom:'#3c3651',sun:'#8aa4ff',fog:'#0b1324',sunX:760,sunY:150}
  };
  const palette = palettes[nightMode ? 'night' : preset];
  ...
  const sunGlow=ctx.createRadialGradient(palette.sunX,palette.sunY,0,palette.sunX,palette.sunY,nightMode?120:190);
  sunGlow.addColorStop(0,palette.sun);
  sunGlow.addColorStop(0.18,nightMode?'rgba(138,164,255,.38)':'rgba(255,245,207,.72)');
  ...
  if(nightMode){
    ...
    for(let index=0;index<90;index+=1){
      const x=(index*137.2)%canvas.width;
      const y=18+((index*71.7)%235);
      const size=index%9===0?1.7:0.8;
  ...
  pmrem.dispose();
  return {background,environment,fogColor:new THREE.Color(palette.fog)};
}
...
function createContactShadowTexture() {
  ...
  const ctx=canvas.getContext('2d');
  if(!ctx) return null;
  const gradient=ctx.createRadialGradient(256,256,18,256,256,250);
  gradient.addColorStop(0,'rgba(20,15,18,.48)');
  gradient.addColorStop(0.35,'rgba(20,15,18,.24)');
  gradient.addColorStop(1,'rgba(20,15,18,0)');
...
function Terrain3D({
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(cameraFov, mount.clientWidth / mount.clientHeight, 0.1, 1000);
    if(cameraMode==='top') camera.position.set(0,24,0.1);
    else if(cameraMode==='walk') camera.position.set(7,2.2,9);
    else if(cameraMode==='architecture') camera.position.set(12.5,7.2,15.5);
    else if(cameraMode==='garden') camera.position.set(4.4,2.6,10.5);
    else camera.position.set(16,13,18);
    ...
    renderer.toneMapping = toneMappingForMode(toneMappingMode);
    renderer.toneMappingExposure = renderExposure * (nightMode ? 0.82 : 1);
    const proceduralEnvironment=createProceduralEnvironment(renderer,environmentPreset,nightMode);
    scene.background=proceduralEnvironment.background;
    scene.environment=proceduralEnvironment.environment;
    const fogNear=THREE.MathUtils.lerp(65,26,renderHaze);
    const fogFar=THREE.MathUtils.lerp(150,68,renderHaze);
    scene.fog=new THREE.Fog(proceduralEnvironment.fogColor,fogNear,fogFar);
    renderer.setPixelRatio(
      Math.min(
        window.devicePixelRatio,
        renderPreset==='photoreal' ? 2.25 :
    ...
    controls.dampingFactor=renderPreset==='photoreal'?0.075:0.1;
    controls.target.set(0, cameraMode==='architecture'?1.4:cameraMode==='garden'?1.1:0, cameraMode==='garden'?-1.4:0);
    controls.maxPolarAngle=Math.PI*0.49;
    controls.minDistance=2.5;
    controls.maxDistance=70;

    const presetLightScale=renderPreset==='editor'?0.82:renderPreset==='photoreal'?1.08:1;
    scene.add(new THREE.HemisphereLight(nightMode ? 0x8aa4d8 : environmentPreset==='golden'?0xffd8b0:0xeaf6ff, nightMode ? 0x172033 : 0x6f7f62, (nightMode ? 0.56 : environmentPreset==='overcast'?0.96:1.12)*presetLightScale));
    scene.add(new THREE.AmbientLight(nightMode ? 0x8396c8 : 0xfff6e9, (nightMode ? 0.14 : environmentPreset==='overcast'?0.42:0.25)*presetLightScale));
    const fillLight = new THREE.DirectionalLight(nightMode ? 0x4d5f8d : environmentPreset==='golden'?0xffc38a:0xffe4c2, (nightMode ? 0.20 : environmentPreset==='overcast'?0.30:0.46)*presetLightScale);
    fillLight.position.set(-18,10,-12);
    scene.add(fillLight);
    const sunColor=nightMode?0x94a3b8:environmentPreset==='golden'?0xffd1a1:environmentPreset==='overcast'?0xdce7ef:0xffffff;
    const sunIntensity=(nightMode?0.30:environmentPreset==='overcast'?0.72:environmentPreset==='golden'?1.08:1.28)*presetLightScale;
    ...
    if (sun.castShadow) {
      const shadowSize = renderPreset==='photoreal' ? 4096 : resolvedPerformance==='quality' ? 2048 : 1024;
      ...
      sun.shadow.normalBias = renderPreset==='photoreal' ? 0.018 : 0.03;
    }
    scene.add(sun);
    if(environmentPreset==='golden' || nightMode){
      const warmAccent=new THREE.PointLight(nightMode?0xffb76b:0xffd0a0,nightMode?34:18,24,2);
      warmAccent.position.set(-5,3.2,4);
      warmAccent.castShadow=renderer.shadowMap.enabled && renderPreset==='photoreal';
      scene.add(warmAccent);
      const coolAccent=new THREE.PointLight(nightMode?0x6f8cff:0xbad8ff,nightMode?16:8,20,2);
      coolAccent.position.set(7,5,-5);
      scene.add(coolAccent);
    }
    if (showGrid3D && renderPreset==='editor') scene.add(new THREE.GridHelper(28, 28, 0x94a3b8, nightMode ? 0x334155 : 0xd7e0eb));
    ...
    const terrainMat = new THREE.MeshStandardMaterial({ vertexColors: true, roughness: renderPreset==='photoreal'?0.84:0.95, metalness: 0, flatShading: false, envMapIntensity:environmentStrength*0.55 });
    ...
    if(contactShadows && renderPreset!=='editor'){
      contactShadowTexture=createContactShadowTexture();
      if(contactShadowTexture){
        const contactPlane=new THREE.Mesh(
          new THREE.PlaneGeometry(20,13),
          new THREE.MeshBasicMaterial({map:contactShadowTexture,transparent:true,depthWrite:false,opacity:renderPreset==='photoreal'?0.46:0.30})
        );
        contactPlane.rotation.x=-Math.PI/2;
        contactPlane.position.y=0.018;
        contactPlane.renderOrder=1;
        scene.add(contactPlane);
      }
    }
    if(reflectiveGround && renderPreset!=='editor'){
      const reflectionPlane=new THREE.Mesh(
        new THREE.PlaneGeometry(23.5,15.5),
        new THREE.MeshPhysicalMaterial({color:0xffffff,transparent:true,opacity:renderPreset==='photoreal'?0.065:0.035,roughness:0.44,metalness:0.05,clearcoat:0.42,clearcoatRoughness:0.62,envMapIntensity:environmentStrength,depthWrite:false})
      );
      reflectionPlane.rotation.x=-Math.PI/2;
      reflectionPlane.position.y=0.026;
      reflectionPlane.renderOrder=2;
    ...
    scene.traverse(node=>{
      ...
      materials.forEach(material=>{
        if(material instanceof THREE.MeshStandardMaterial || material instanceof THREE.MeshPhysicalMaterial){
          material.envMapIntensity=environmentStrength;
    ...
    const captureRender=()=>{
      ...
      renderer.domElement.toBlob(blob=>{
        if(!blob){onStatusRef.current('Renderbild konnte nicht erzeugt werden.');return;}
        const url=URL.createObjectURL(blob);
        const link=document.createElement('a');
        link.href=url;
        link.download=`al-green-design-render-${new Date().toISOString().slice(0,10)}.png`;
        link.click();
        window.setTimeout(()=>URL.revokeObjectURL(url),1200);
        onStatusRef.current('3D-Render wurde als PNG gespeichert.');
      },'image/png');
    };
    window.addEventListener('algreen-capture-render',captureRender);
    ...
    return () => {
      disposed = true;
      window.removeEventListener('algreen-capture-render',captureRender);
      proceduralEnvironment.background.dispose();
      proceduralEnvironment.environment.dispose();
  ...
  }, [terrainBlobs, elevationPoints, zones, objects, levels, rooms, importedModels, selectedImportedModelId, selectedId, selectedKind, nightMode, growthYear, season, sunAzimuth, sunElevation, showContours, showGrid3D, enableShadows, performanceMode, cameraMode, renderPreset, environmentPreset, toneMappingMode, renderExposure, environmentStrength, cameraFov, contactShadows, reflectiveGround, renderHaze]);
