...
function generatedEntity(entity: Omit<CadEntity, "id" | "visible" | "locked">): CadEntity {
  return {
    ...
    locked: false,
    ...entity,
    metadata: { ...(entity.metadata ?? {}), generatedBy: "Garden AI Alpha 6" }
  };
}
...
export function interpretGardenCommand(command: string, project: ProjectState): GardenAIResult {
  ...
  if (!created.length) {
    ...
    created.push(...concept.entities.map(entity => ({ ...entity, metadata: { ...(entity.metadata ?? {}), generatedBy: "Garden AI Alpha 6" } })));
  }
